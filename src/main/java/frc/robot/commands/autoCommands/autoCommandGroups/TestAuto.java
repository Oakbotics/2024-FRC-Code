...
public class TestAuto extends SequentialCommandGroup {
   ...
   */
  public TestAuto(DriveSubsystem driveSubsystem, ShooterSubsystem shooterSubsystem, ConveyorSubsystem conveyorSubsystem, PnuematicSubsystem pnuematicSubsystem, NoteLimelightSubsystem noteLimelightSubsystem) {
    ...
    if (DriverStation.getAlliance().get() == DriverStation.Alliance.Red) {
        initialPose = PathPlannerPath.fromPathFile("AmpSideToMiddleWoofer").flipPath().getPreviewStartingHolonomicPose();
        isRedAlliance = true;
      }
      else {
        initialPose = PathPlannerPath.fromPathFile("AmpSideToMiddleWoofer").getPreviewStartingHolonomicPose();
        isRedAlliance = false;
      }
    final Pose2d initialPose2D = initialPose;
    addCommands(
      new InstantCommand(() -> m_driveSubsystem.resetOdometry(initialPose2D)),
      new InstantCommand(() -> m_driveSubsystem.resetEncoders()),
      new ParallelRaceGroup(
        new RunCommand(() -> m_driveSubsystem.findPathToPose(3.2, 5.6 , 0, isRedAlliance)),
        new SensorIntakeCommand(conveyorSubsystem, shooterSubsystem, true)
      ),
      new ParallelRaceGroup(
        new RunCommand(() -> m_driveSubsystem.findPathToPose(3.2, 5.6 , 0, isRedAlliance)),
