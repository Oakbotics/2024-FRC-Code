...
public class M_4PRevBlue extends SequentialCommandGroup {
   ...
   */
  public M_4PRevBlue(DriveSubsystem driveSubsystem, ShooterSubsystem shooterSubsystem, ConveyorSubsystem conveyorSubsystem, PnuematicSubsystem pnuematicSubsystem) {
      ...
      new SequentialCommandGroup(
        ...
        new ParallelCommandGroup(
          new GoToAutoPositionCommand(m_driveSubsystem, ()-> AutoConstants.middleStartingPose).withTimeout(1.5),
           new ShootCommand(shooterSubsystem).withTimeout(1.5)
        ),
        new ShootCommandGroup(m_conveyorSubsystem, m_shooterSubsystem),

        new GoToAutoPositionCommand(m_driveSubsystem, ()-> AutoConstants.blueAmpNoteLineupPose),

        new ParallelCommandGroup(
          new SensorIntakeCommand(m_conveyorSubsystem, m_shooterSubsystem, true).withTimeout(3),
          new GoToAutoPositionCommand(m_driveSubsystem, ()-> new Pose2d(AutoConstants.bC1Pose.getTranslation(), Rotation2d.fromDegrees(0))).withTimeout(2.5)
        ),
        ...
        new GoToAutoPositionCommand(m_driveSubsystem, ()-> AutoConstants.bluePodiumNoteLineupPose).alongWith(new SensorIntakeCommand(m_conveyorSubsystem, m_shooterSubsystem, false).withTimeout(1)),
        new ParallelCommandGroup(
          new SensorIntakeCommand(m_conveyorSubsystem, m_shooterSubsystem, true).withTimeout(3),
          new GoToAutoPositionCommand(m_driveSubsystem,()->  new Pose2d(AutoConstants.bC3Pose.getTranslation(), Rotation2d.fromDegrees(0))).withTimeout(3)
        ),
        new ParallelCommandGroup(
          new GoToAutoPositionCommand(m_driveSubsystem, ()-> AutoConstants.subwooferShootSafe).withTimeout(1.5),
          new ShootCommand(shooterSubsystem).withTimeout(1.5)
